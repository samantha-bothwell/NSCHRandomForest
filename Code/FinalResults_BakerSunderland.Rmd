...
#Corr gram plot one
library(corrgram)
corrgram(DO_1b, order=NULL, lower.panel=panel.shade, upper.panel=NULL, text.panel=panel.txt, main="Psychoecological Variables", use = "pairwise.complete.obs")
...
library(corrgram)
DO_1ba <- DO_1b %>% 
  select(SC_AGE_YEARS, SC_HISPANIC_R, SC_RACE_R, SC_SEX, PHYSACTIV, SCREENTIME, 
         HOURSLEEP, K2Q32A, K2Q33A, ACE1, ACE3, ACE4, ACE6, ACE9, ACE10, A1_BORN, A1_PHYSHEALTH, 
         BIRTHWT, BULLIED_R, CURRCOV, FPL_I1, HIGRADE_TVIS, K10Q11, K10Q22, K10Q40_R, K7Q30)
DO_1a <- rename(DO_1a, c("SC_AGE_YEARS"="Age", "SC_HISPANIC_R"="Hispanic_Identity","SC_RACE_R"="Racial_Identity","SC_SEX"="Sex","PHYSACTIV"="Physical_Activity_Level","SCREENTIME"="Screentime","HOURSLEEP"="Hours_Sleeping","K2Q32A"="Depression","K2Q33A"="Anxiety","ACE1"="Food_Insecurity","ACE3"="Parental_Divorce","ACE4"="Parental_Death","ACE6"="Domestic_Violence_Witnessed","ACE9"="Drug_or_Alcohol_Abuse","ACE10"="Racial_Discrimination","A1_BORN"="Parent_Immigration_Status","A1_PHYSHEALTH"="Parent_Physical_Health","BIRTHWT"="Birth_Weight","BULLIED_R"="Bullied_Status","CURRCOV"="Healthcare_Coverage","FPL_I1"="Family_Poverty_Ratio","HIGRADE_TVIS"="Parent_Education_Level","K10Q11"="Neighborhood_Walkability","K10Q22"="Rundown_Housing","K10Q40_R"="Neighborhood_Safety","K7Q30"="Sports_Involvement"))

DO_1a <- DO_1a[-c(1,2,7,15,17,18,26,28,31,34:36,38:43)] #removing year, fips, and old BMICLASS variable
```